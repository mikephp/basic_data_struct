 ...
 * Desc: ......................... Console Freecap version
  ...
  $Id: FreecapCon.dpr,v 1.4 2005/05/12 04:21:22 bert Exp $

  $Log: FreecapCon.dpr,v $
...
begin
   ...
   begin
        WriteLn('FreeCap v' + FREECAP_VERSION + ' console version. Copyright (c) 2005 by Max Artemev'#13#10);
        WriteLn(ExtractFileName(ParamStr(0)) + ' <options> <program.exe> "program parameters"');
        WriteLn('Where options are:');
        WriteLn('    -b <base path> '#9'Path where FreeCap is installed, Default is $(APPDATA)\FreeCap');
        WriteLn('    -f <config.xml> '#9'Path to SOCKS config file. Default is $(WINDIR)\freecap.xml');
        WriteLn('    -w <workdir>'#9'Set the working directory of running program to <workdir>.');
        WriteLn('       Use double quotas if directory contains spaces. If omitted, working');
        WriteLn('       directory will be set to program path');
        WriteLn('    -o <logfile.txt>'#9'Set the logfile to <logfile.txt>');
        WriteLn('    -u <library.dll>'#9'Use specified library to inject into process');
        WriteLn('    -l <loglevel>'#9'Set the log level. Log level can be:');
        WriteLn('    '#9'0 - No log');
        WriteLn('    '#9'1 - Injection log');
        WriteLn('    '#9'2 - Connection status log');
        WriteLn('    '#9'4 - SOCKS status log');
        WriteLn('    '#9'8 - Warning messages');
        WriteLn('    '#9'15 - All');
        exit;
   end;
   ...
   begin
        ...
        begin
             ...
             begin
                  WriteLn('Base path wasn''t specified');
                  exit;
             end;
             BasePath := ParamStr(i);
        end
        ...
        begin
             ...
             if i > ParamCount then
             begin
                  WriteLn('Config file wasn''t specified');
                  exit;
             end;
        ...
        begin
             ...
             if i > ParamCount then
             begin
                  WriteLn('Working directory wasn''t specified');
                  exit;
             end;
             WorkDir := ParamStr(i);
        end
        ...
        begin
             ...
             if i > ParamCount then
             begin
                  WriteLn('Log file wasn''t specified');
                  exit;
             end;
             logfile := ParamStr(i);
        end
        ...
        begin
             ...
             if i > ParamCount then
             begin
                  WriteLn('Log level wasn''t specified');
                  exit;
             end;
             loglevel := ParamStr(i);
             if not (StrToIntDef(loglevel, -1) in [0..15]) then
             begin
                  WriteLn('Log level is not a number in range 0 to 15');
                  exit;
             end;
        end
        ...
        begin
             ...
             if i > ParamCount then
             begin
                  WriteLn('Library wasn''t specified');
                  exit;
             end;
             LibName := ParamStr(i);
        end
   ...
   if ProgramName = '' then
   begin
        WriteLn('Program to execute wasn''t specified');
        exit;
   end;
