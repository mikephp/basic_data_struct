 ...
 * Desc: ...................... SOCKS v4 client side proxy implementation
 ...
 $Id: Socks4Proxy.pas,v 1.4 2005/10/31 14:26:22 bert Exp $

 $Log: Socks4Proxy.pas,v $
 Revision 1.4  2005/10/31 14:26:22  bert
 *** empty log message ***
...
begin
     ...
     begin
          inc(i, 3);
          Buf[i] := #1; // non-zero byte indicating that hostname follows after USERID (SOCKS4a)
          inc(i);
     end
     else
     begin
     ...
     if (Flogin = '') then
     begin
          UserBufSize := SizeOf(UserBuf);
          if GetUserName(@UserBuf, UserBufSize) then
             Flogin := UserBuf;
     end;
     ...
     begin
          ...
          if not isPseudoDestAddr() then
            inc(i, Length(Flogin))
          else
          begin
               host := DestAddrHost;
               inc(i, Length(Flogin) + 1);
