 ...
 * Desc: ...................... SOCKSv5 client side proxy implementation
 ...
 $Id: Socks5Proxy.pas,v 1.6 2005/05/27 12:45:47 bert Exp $

 $Log: Socks5Proxy.pas,v $
 Revision 1.6  2005/05/27 12:45:47  bert
 *** empty log message ***

 Revision 1.5  2005/05/16 04:31:52  bert
 *** empty log message ***
...
uses Windows, Messages, Classes, SysUtils, direct_addr, dns,
     winsock2, loger, misc, cfg, AbstractProxy, stub;
...
begin
     ...
     Log(LOG_LEVEL_SOCKS, 'Attempting make TCP connection to (%s:%d)...',[GetAddrString(FDestAddr.sin_addr.S_addr), ntohs(FDestAddr.sin_port)]);
     ...
     if (buf[0] = SOCKS5_REP_VER) and (buf[1] = SOCKS5_REP_REP_OK) then
     begin
          Log(LOG_LEVEL_SOCKS, 'Connection established',[]);
          result := 0;
     end
     else
     begin
          ...
          Log(LOG_LEVEL_SOCKS, 'Connection failure: %s %s',[s, buf]);
          result := -1;
     end;
end;
...
begin
     ...
     Stub_getsockname(Fs2, BndAddr, BndAddrLen);

     if (BndAddr.sin_port = 0) then
     begin
          Stub_Bind(Fs2, @BndAddr, BndAddrLen);
          Stub_GetSockName(Fs2, BndAddr, BndAddrLen);
     ...
     begin
          if ((BndAddr.sin_addr.S_addr = 0) or IsLANAddr(BndAddr.sin_addr.S_addr)) and (not FLanProxy) then
            Log(LOG_LEVEL_WARN, 'You''re trying to establish tunnel for UDP stream, but you''re using external proxy. It will not work.', []);
...
//     (buffer + 3)^ := SOCKS5_REQ_ATYP_IP4;
     ...
     result := Stub_Sendto(FS2, buffer, size, flags, FDestAddr, SizeOf(FDestAddr));
     if result = -1 then
